...
with st.sidebar:
    ...
    if all_names:
        ...
        chosen = st.selectbox("Select Bakery", all_names, index=idx, key="sidebar_bakery_select")
        st.session_state.selected_bakery = chosen
        
        # 🛠️ REBUILT FLAVOR LOGIC
        b_rows = df_clean[df_clean['Bakery Name'] == chosen]
        raw_flavs = b_rows['Fastelavnsbolle Type'].unique()
        
        # Remove "Wishlist" and numbers from the dropdown
        flavs = sorted([
            str(f).strip() for f in raw_flavs 
            if f and str(f).strip() 
            and not str(f).isdigit() 
            and str(f).lower() != "wishlist"
        ])
        
        f_sel = st.selectbox("Flavor", flavs + ["➕ New..."], key=f"f_sel_drop_{chosen}")
        
        if f_sel == "➕ New...":
            f_name = st.text_input("New flavor name:", key=f"f_text_in_{chosen}")
        else:
            f_name = f_sel
        ...
        if mode == "Rate it":
            ...
            if st.button("Submit ✅", use_container_width=True):
                ...
                st.toast(f"Success! Rated {chosen}", icon='🥯')
                st.cache_data.clear()
                time.sleep(1)
                st.rerun()
        else:
