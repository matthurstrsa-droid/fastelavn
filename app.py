...
geolocator = Nominatim(user_agent="bakery_explorer_v5")
...
wishlist_bakeries = df[df['Rating'] == 0.1]['Bakery Name'].unique().tolist()
...
with st.sidebar:
    ...
    # Check for selection from Map OR Checklist
    clicked_bakery = st.session_state.get("clicked_bakery", None)
    list_selection = st.session_state.get("list_selection", None)
    
    # Priority: List Selection > Map Click
    target_bakery = list_selection if list_selection else clicked_bakery
    ...
    if st.button("Submit"):
        try:
            ...
            st.session_state.list_selection = None
            st.rerun()
        except Exception as e:
            st.error(f"Error: {e}")
...
with tab1:
    ...
    for _, row in display_df.iterrows():
        ...
        is_wish = row['Bakery Name'] in wishlist_bakeries and not is_tried
        ...
        popup_html = f"<b>{row['Bakery Name']}</b><br>Rating: {row['Rating'] if row['Rating'] > 0.1 else 'Wishlist'}"
        folium.Marker(
...
with tab2:
    st.subheader("Your Bakery Tracker")
    
    # Prepare list data
    checklist_df = display_df.groupby('Bakery Name').agg({'Neighborhood': 'first', 'Rating': 'max'}).reset_index()
    ...
    # Interaction: Selection box to "pick" a bakery from the list
    selection = st.selectbox("Select a bakery to Rate or Wishlist:", 
                             ["-- Select from List --"] + sorted(checklist_df['Bakery Name'].tolist()))
    
    if selection != "-- Select from List --":
        st.session_state.list_selection = selection
        st.info(f"👉 **{selection}** selected! Use the sidebar to update your rating.")

    st.dataframe(checklist_df[['Status', 'Bakery Name', 'Neighborhood']].sort_values('Status'), 
